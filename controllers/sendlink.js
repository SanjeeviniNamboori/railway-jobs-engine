...
class SendLink{
    ...
    sendVerificationLink(req,res,pool,id){
     //  console.log("In send verification link" +id ) ;
        //console.log("dfabfb" + id);
        pool.getConnection((error,connection)=>{
            if(connection){
                connection.query('select usercontact.contactinfo,login.actoken from usercontact inner join login on login.uid=? and usercontact.uid=? and usercontact.ctypeid=? ',[id,id,'4'],(error,result)=>{
                    if(result){
                      //  console.log("in send verification link " + result[0].actoken);
