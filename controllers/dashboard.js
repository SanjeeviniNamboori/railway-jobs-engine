...
class Dashboard {
    ...
    //    var authentication_token = req.session.authenticationtoken;
        ...
        console.log("in dashboard" + authentication_token+ sess_id);
...
//connection.query('select * from user inner join usercontact inner join profileimage on user.uid =? and usercontact.uid=? and profileimage.uid=?',[sess_id,sess_id,sess_id],(err1,res1)=>{
                          if(res1.length>0){
                                //console.log(res1);
                                 for(let i=0; i<res1.length;i++){
                                     ...
                                     // data.userProfileImage = res1[i].profileimage;
...
//                              res.status(200).send({data:data,state:"dashboard"});
        ...
        res.status(200).send({error:"wrong username/password"});
                     ...
                     console.log("data dasboard" + JSON.stringify(data));
